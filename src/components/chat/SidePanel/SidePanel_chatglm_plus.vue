...
import { ROUTE_CHATGLM_PLUS_CONVERSATION, ROUTE_CHATGLM_PLUS_CONVERSATION_NEW } from '@/router/constants';
import { chatOperator } from '@/operators';
import { IChatConversation } from '@/models';
import { Status } from '@/models';
import { CHATGLM_PLUS_SERVICE_ID } from '@/constants';
  ...
  methods: {
    ...
    async onConfirm(conversation: IChatConversation) {
      ...
      if (conversation?.deleting) {
        await chatOperator.deleteConversation(conversation.id, 'chatglm_plus', {
          token
        });
        await this.$store.dispatch('chat/getConversations', CHATGLM_PLUS_SERVICE_ID);
      } else if (conversation?.editing) {
        await chatOperator.updateConversation(conversation, 'chatglm_plus', {
          token
        });
        await this.$store.dispatch('chat/getConversations', CHATGLM_PLUS_SERVICE_ID);
      } else {
