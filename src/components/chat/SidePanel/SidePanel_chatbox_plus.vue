...
import { ROUTE_ChatBOT_o1_CONVERSATION, ROUTE_ChatBOT_o1_CONVERSATION_NEW } from '@/router/constants';
import { chatOperator } from '@/operators';
import { IChatConversation } from '@/models';
import { Status } from '@/models';
import { CHATBOT_o1_SERVICE_ID } from '@/constants';
  ...
  methods: {
    ...
    async onConfirm(conversation: IChatConversation) {
      ...
      if (conversation?.deleting) {
        await chatOperator.deleteConversation(conversation.id, 'chatbox_plus', {
          token
        });
        await this.$store.dispatch('chat/getConversations', CHATBOT_o1_SERVICE_ID);
      } else if (conversation?.editing) {
        await chatOperator.updateConversation(conversation, 'chatbox_plus', {
          token
        });
        await this.$store.dispatch('chat/getConversations', CHATBOT_o1_SERVICE_ID);
      } else {
