...
import { ROUTE_DeepSeek_CONVERSATION, ROUTE_DeepSeek_CONVERSATION_NEW } from '@/router/constants';
  ...
  methods: {
    ...
    async onConfirm(conversation: IChatConversation) {
      ...
      if (conversation?.deleting) {
        await chatOperator.deleteConversation(conversation.id, 'deepseek', {
          token
        });
        await this.$store.dispatch('chat/getConversations', DEEPSEEK_SERVICE_ID);
      } else if (conversation?.editing) {
        await chatOperator.updateConversation(conversation, 'deepseek', {
          token
        });
        await this.$store.dispatch('chat/getConversations', DEEPSEEK_SERVICE_ID);
      } else {
