...
import { ROUTE_CHATBOT_4o_CONVERSATION, ROUTE_CHATBOT_4o_CONVERSATION_NEW } from '@/router/constants';
import { chatOperator } from '@/operators';
import { IChatConversation } from '@/models';
import { Status } from '@/models';
import { CHATBOT_4o_SERVICE_ID } from '@/constants';
  ...
  methods: {
    ...
    async onConfirm(conversation: IChatConversation) {
      ...
      if (conversation?.deleting) {
        await chatOperator.deleteConversation(conversation.id, 'chatbox', {
          token
        });
        await this.$store.dispatch('chat/getConversations', CHATBOT_4o_SERVICE_ID);
      } else if (conversation?.editing) {
        await chatOperator.updateConversation(conversation, 'chatbox', {
          token
        });
        await this.$store.dispatch('chat/getConversations', CHATBOT_4o_SERVICE_ID);
      } else {
