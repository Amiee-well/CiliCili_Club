<template>
  <div class="panel">
    ...
    <div v-else class="conversations">
      <div class="conversation" @click="onExpend(false)">
        <div class="icons">
          <font-awesome-icon icon="fa-solid fa-toggle-on" class="icon" />
        </div>
        <div class="title">
          {{ $t('chat.message.expend') }}
        </div>
      </div>
        ...
        >
          <div class="title">
            ...
            <span v-else-if="conversation?.title || conversation?.messages">{{
              conversation?.title || new Date(conversation?.updated_at * 1000).toLocaleString()
            }}</span>
          </div>
...
import { ROUTE_CHATGLM_CONVERSATION, ROUTE_CHATGLM_CONVERSATION_NEW } from '@/router/constants';
import { chatOperator } from '@/operators';
import { IChatConversation } from '@/models';
import { Status } from '@/models';
import { CHATGLM_SERVICE_ID } from '@/constants';
...
export default defineComponent({
  ...
  computed: {
    ...
    conversationGroups() {
      ...
      console.log(this.$store.state.chat.conversations);
  ...
  methods: {
    ...
    async onConfirm(conversation: IChatConversation) {
      ...
      if (conversation?.deleting) {
        await chatOperator.deleteConversation(conversation.id, 'ChatGLM', {
          token
        });
        await this.$store.dispatch('chat/getConversations', CHATGLM_SERVICE_ID);
      } else if (conversation?.editing) {
        await chatOperator.updateConversation(conversation, 'ChatGLM', {
          token
        });
        await this.$store.dispatch('chat/getConversations', CHATGLM_SERVICE_ID);
      } else {
