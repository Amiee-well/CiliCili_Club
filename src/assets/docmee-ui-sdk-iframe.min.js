...
function D(e) {
  try {
    ...
    let r = e.match(F) || [];
    for (let t = 1; t < r.length; t++) (e = l(r, t).join('')), (r = e.match(F) || []);
    return e;
  }
}
function L(e) {
  ...
  for (const a of n) e = e.replace(new RegExp(a, 'g'), r[a]);
  return e;
}
function P(e) {
...
const H = (e) => e == null,
  T = (e) => encodeURIComponent(e).replaceAll(/[!'()*]/g, (r) => `%${r.charCodeAt(0).toString(16).toUpperCase()}`),
...
function B(e) {
  switch (e.arrayFormat) {
    case 'index':
      return (r) => (t, n) => {
        const a = t.length;
        return n === void 0 || (e.skipNull && n === null) || (e.skipEmptyString && n === '')
          ? t
          : n === null
          ? [...t, [f(r, e), '[', a, ']'].join('')]
          : [...t, [f(r, e), '[', f(a, e), ']=', f(n, e)].join('')];
      };
    case 'bracket':
      return (r) => (t, n) =>
        n === void 0 || (e.skipNull && n === null) || (e.skipEmptyString && n === '')
          ? t
          : n === null
          ? [...t, [f(r, e), '[]'].join('')]
          : [...t, [f(r, e), '[]=', f(n, e)].join('')];
    ...
    case 'bracket-separator': {
      ...
      return (t) => (n, a) =>
        a === void 0 || (e.skipNull && a === null) || (e.skipEmptyString && a === '')
          ? n
          : ((a = a === null ? '' : a),
            n.length === 0 ? [[f(t, e), r, f(a, e)].join('')] : [[n, f(a, e)].join(e.arrayFormatSeparator)]);
    }
    default:
      return (r) => (t, n) =>
        n === void 0 || (e.skipNull && n === null) || (e.skipEmptyString && n === '')
    ...
    case 'separator':
      return (t, n, a) => {
        const i = typeof n == 'string' && n.includes(e.arrayFormatSeparator),
          s = typeof n == 'string' && !i && d(n, e).includes(e.arrayFormatSeparator);
        n = s ? d(n, e) : n;
...
function w(e, r) {
  return (
    r.parseNumbers && !Number.isNaN(Number(e)) && typeof e == 'string' && e.trim() !== ''
      ? (e = Number(e))
      : r.parseBooleans &&
        e !== null &&
...
function g(e, r) {
  ...
  for (const a of e.split('&')) {
    ...
    let [s, o] = M(i, '=');
    s === void 0 && (s = i),
      (o = o === void 0 ? null : ['comma', 'separator', 'bracket-separator'].includes(r.arrayFormat) ? o : d(o, r)),
      t(d(s, r), o, n);
  }
  for (const [a, i] of Object.entries(n))
    if (typeof i == 'object' && i !== null) for (const [s, o] of Object.entries(i)) i[s] = w(o, r);
    else n[a] = w(i, r);
  return r.sort === !1
    ? n
    : (r.sort === !0 ? Object.keys(n).sort() : Object.keys(n).sort(r.sort)).reduce((a, i) => {
        const s = n[i];
...
function C(e, r) {
  ...
  (r = {
    ...
    E(r.arrayFormatSeparator);
  const t = (s) => (r.skipNull && H(e[s])) || (r.skipEmptyString && e[s] === ''),
    n = B(r),
    a = {};
  for (const [s, o] of Object.entries(e)) t(s) || (a[s] = o);
  const i = Object.keys(a);
  return (
    r.sort !== !1 && i.sort(r.sort),
    i
      .map((s) => {
        const o = e[s];
        return o === void 0
          ...
          ? f(s, r)
          : Array.isArray(o)
          ? o.length === 0 && r.arrayFormat === 'bracket-separator'
            ? f(s, r) + '[]'
            : o.reduce(n(s), []).join('&')
          : f(s, r) + '=' + f(o, r);
      })
...
class J {
   ...
   */
  constructor({ token: r, page: t = 'dashboard', container: n, pptId: a, onMessage: i, DOMAIN: s, ...o }) {
    ...
    (this.onMessage = i),
      ...
      r || console.log('%c 初始化时，token不能为空！', 'color: #d7514f; background-color: #2e2e2e'),
      s && (j.BASE_URL = s),
      this.init({ token: r, page: t, pptId: a, ...o });
  }
  _postMessage(r) {
    var t, n;
  ...
  init({ token: r, page: t = 'dashboard', ...n }) {
    if (t === 'editor' && !n.pptId) throw new Error('初始化editor页面时，必须传入pptId');
    (this.query = Object.assign({}, this.query, n)),
      (this.docmeeHref = S(t)),
      this.updateToken(r),
      this._initIframe(!0);
  }
  ...
  _initIframe(r) {
    ...
    (n.src = i),
      ...
      n.setAttribute('allow', 'clipboard-write'),
      (this.iframe = n),
      (this.iframeMounted = !1),
      (t.innerHTML = ''),
      t.appendChild(n),
        ...
        setTimeout(() => {
          this.iframeMounted ||
            (this.initInterval = setInterval(() => {
              if (this.iframeMounted || s >= 5) return (s = 0), clearInterval(this.initInterval);
              r &&
                this._postMessage({
                  type: 'transParams',
                  data: this.query
          ...
          window.addEventListener('message', async (o) => {
            ...
            if (
              (r &&
                (c.type === 'mounted' || c.type === 'invalid-token') &&
                ((this.iframeMounted = !0),
                c.type === 'mounted' &&
                  this._postMessage({
                    type: 'transParams',
                    data: this.query
                  })),
              c.type === 'user-info' && (this.iframeMounted = !0),
              c.type.startsWith('before'))
            ) {
              const N = await ((y = this.onMessage) == null ? void 0 : y.call(this, c));
              this._postMessage({ data: N, type: `recover_${c.type}` });
            } else (p = this.onMessage) == null || p.call(this, c);
          });
      });
  }
  /**
   ...
   */
  updateToken(r) {
    /(a|s)k_.+/.test(r) || console.error('token 错误！'),
      (this.token = r),
      (this.query.token = r),
      this.iframeMounted &&
        this._postMessage({
