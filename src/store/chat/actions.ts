...
export const setConversation = async ({ commit, state }: any, payload: { id: string }): Promise<void> => {
  log(setConversation, 'set conversation', payload);
  const conversations = state.conversations;
  const index = conversations?.findIndex((conversation: { id: string }) => conversation.id === payload.id);
  if (index > -1) {
...
export const getApplication = async (
  { commit, state, rootState }: ActionContext<IChatState, IRootState>,
  serviceId: string
